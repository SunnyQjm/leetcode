...
- 思路：
  ...
  - 首先对输入的数组进行排序；
  - 从左往右，依次遍历；（遇到和上轮遍历相同的元素直接跳过）
  - 每次固定当前值，并对当前值右侧的剩余数据执行2sum操作，具体如下：
    - 假设固定值为a，则2sum的目标值(target)为-a；
    - 接着参考前面讨论过的2sum实现执行2sum操作：
      - 用一个字典记录已经访问过的值及其下标；
      - 如果访问到某个值num[i]是，发现其匹配值target - nums[i] 出现在dict里面，表示其左边有一个是可以与nums[i]想加得target，此时返回[dict[nums[i]], i]即为结果；
