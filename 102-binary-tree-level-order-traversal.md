...
- 思路：
  ...
  - 采用图的BFS遍历的思想对树进行逐层遍历；
  - 用queue来实现BFS；（由于这边是树型结构，所以不会有环路，故不需要用visited来记录哪些节点已经访问过）
  - 为了记录当前遍历的节点在哪一层，我们用两个变量curLevelNum和nextLevelNum来分别记录当前层次还有多少没有遍历，以及下一层次还有多少没有遍历。
